...
public class DispAuxActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (display.getMode().getPhysicalWidth() == 1280 && display.getMode().getPhysicalHeight() == 480) {
    ...
    private void setPlayButtonStatus(boolean playing) {
        if (playing) {
            ((ImageView) findViewById(R.id.music_button_play)).getDrawable().setLevel(1);
        } else {
            ((ImageView) findViewById(R.id.music_button_play)).getDrawable().setLevel(0);
        }
    }
    ...
    private void startReceiver() {
        ...
        getContentResolver().registerContentObserver(Settings.Global.getUriFor(MachineConfig.KEY_DISPAUX_APPAUTO_DISABLE), true, mDispAuxAppAutoObserver);
    ...
    public static void startDisplayAux(Context context) {
        if (Util.isAndroidQ() && (Util.isPX30() || Util.isPX6()) && MachineConfig.getPropertyIntReadOnly(MachineConfig.KEY_DISPAUX_ENABLE) == 1 && Utils.getDispAuxInfo(context) != null) {
            try {
                ActivityOptions options = ActivityOptions.makeBasic();
                options.setLaunchDisplayId(1);
