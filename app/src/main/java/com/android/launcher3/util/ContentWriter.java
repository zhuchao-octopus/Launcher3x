...
public class ContentWriter {
    ...
    public ContentValues getValues(Context context) {
        ...
        if (mIcon != null && !LauncherAppState.getInstance(context).getIconCache().isDefaultIcon(mIcon, mUser)) {
    ...
    public int commit() {
        if (mCommitParams != null) {
            return mContext.getContentResolver().update(mCommitParams.mUri, getValues(mContext), mCommitParams.mWhere, mCommitParams.mSelectionArgs);
        }
        return 0;
    }
