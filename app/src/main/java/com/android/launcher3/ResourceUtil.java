...
public class ResourceUtil {
	...
	public static String updateUi(Context context) { // only launcher use now
		...
		String value = MachineConfig
				.getPropertyReadOnly(LAUNCHER_UI);
		if (value == null){
			value = MachineConfig
		...
		} else if (MachineConfig.VALUE_SYSTEM_UI32_KLD8.equals(value)||
				MachineConfig.VALUE_SYSTEM_UI34_KLD9.equals(value)||
				MachineConfig.VALUE_SYSTEM_UI35_KLD813.equals(value)) {
	...
	public static void loadGernalAppRes(Context context) {
		...
		if (id > 0)
			mGeneralAppIconBgWidth = (int) context.getResources().getDimension(id);
		
		id = context.getResources().getIdentifier("app_icon_bg_height", "dimen", context.getPackageName());
		if (id > 0)
			mGeneralAppIconBgHeight = (int) context.getResources().getDimension(id);
		
		id = context.getResources().getIdentifier("app_icon_offset_left", "dimen", context.getPackageName());
		if (id > 0)
			mGeneralAppIconOffsetLeft = (int) context.getResources().getDimension(id);
		
		id = context.getResources().getIdentifier("app_icon_offset_top", "dimen", context.getPackageName());
		if (id > 0)
			mGeneralAppIconOffsetTop = (int) context.getResources().getDimension(id);
		
		id = context.getResources().getIdentifier("app_iocn_text_position_x", "dimen", context.getPackageName());
		if (id > 0)
			mGeneralAppTextPositionX = (int) context.getResources().getDimension(id);
		
		id = context.getResources().getIdentifier("app_iocn_text_position_y", "dimen", context.getPackageName());
		if (id > 0)
			mGeneralAppTextPositionY = (int) context.getResources().getDimension(id);

		id = context.getResources().getIdentifier("app_icon_text_size", "dimen", context.getPackageName());
		if (id > 0)
			mGeneralAppTextSize = (int) context.getResources().getDimension(id);

		Log.d("ResourceUtil", "[" + mGeneralAppIconBgWidth
				+ "," + mGeneralAppIconBgHeight + "]["
				+ mGeneralAppIconOffsetLeft + "," + mGeneralAppIconOffsetTop
				+ "][" + mGeneralAppTextPositionX + "," + mGeneralAppTextPositionY + "," + mGeneralAppTextSize + "]");
	}
}
