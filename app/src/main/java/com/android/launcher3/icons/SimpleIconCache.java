...
public class SimpleIconCache extends BaseIconCache {
    ...
    protected long getSerialNumberForUser(UserHandle user) {
        synchronized (mUserSerialMap) {
            int userIdentifier = user.hashCode();
            int index = mUserSerialMap.indexOfKey(userIdentifier);///user.getIdentifier()
            if (index >= 0) {
                return mUserSerialMap.valueAt(index);
            }
            long serial = mUserManager.getSerialNumberForUser(user);
            mUserSerialMap.put(userIdentifier, serial);
            return serial;
        }
    }
    ...
    protected boolean isInstantApp(ApplicationInfo info) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) { // 检查是否是 API 26 及以上版本
            ///return info.isInstantApp();
            return false; // 或者其他适当的逻辑
        } else {
            // 处理低于 API 26 的情况
            return false; // 或者其他适当的逻辑
        }
