...
public class BgDataModel {
    ...
    private synchronized void dumpProto(String prefix, FileDescriptor fd, PrintWriter writer, String[] args) {
        ...
        if (args.length > 1 && TextUtils.equals(args[1], "--debug")) {
            ...
            // 创建 LauncherImpression 的 Builder
            LauncherDumpProto.LauncherImpression.Builder impressionBuilder = LauncherDumpProto.LauncherImpression.newBuilder();
            // 将 targetList 转换为数组并设置到 Builder 中
            for (DumpTarget target : targetList) {
                impressionBuilder.addTargets(target);
            }
            // 构建最终的 LauncherImpression 对象
            LauncherDumpProto.LauncherImpression proto = impressionBuilder.build();
            try (FileOutputStream fos = new FileOutputStream(fd)) {
                // 将 proto 对象转换为字节数组并写入文件
                byte[] byteArray = proto.toByteArray();
                fos.write(byteArray);
                Log.d(TAG, byteArray.length + " Bytes");
            } catch (IOException e) {
