...
public class FolderIcon extends FrameLayout implements FolderListener {
    ...
    public static final int NUM_ITEMS_IN_PREVIEW = FeatureFlags.LAUNCHER3_LEGACY_FOLDER_ICON ? StackFolderIconLayoutRule.MAX_NUM_ITEMS_IN_PREVIEW : ClippedFolderIconLayoutRule.MAX_NUM_ITEMS_IN_PREVIEW;
    ...
    private void init() {
        ...
        mPreviewLayoutRule = FeatureFlags.LAUNCHER3_LEGACY_FOLDER_ICON ? new StackFolderIconLayoutRule() : new ClippedFolderIconLayoutRule();
    ...
    private boolean willAcceptItem(ItemInfo item) {
        ...
        return ((itemType == LauncherSettings.Favorites.ITEM_TYPE_APPLICATION || itemType == LauncherSettings.Favorites.ITEM_TYPE_SHORTCUT || itemType == LauncherSettings.Favorites.ITEM_TYPE_DEEP_SHORTCUT) && !mFolder.isFull() && item != mInfo && !mFolder.isOpen());
    }
    ...
    private void onDrop(final ShortcutInfo item, DragView animateView, Rect finalRect, float scaleRelativeToDragLayer, int index, Runnable postAnimationRunnable) {
        item.cellX = -1;
        item.cellY = -1;
        ...
        if (animateView != null) {
            ...
            final PreviewItemDrawingParams params = index < mDrawingParams.size() ? mDrawingParams.get(index) : null;
    ...
    private void computePreviewDrawingParams(int drawableSize, int totalSize) {
        if (mIntrinsicIconSize != drawableSize || mTotalWidth != totalSize || mPrevTopPadding != getPaddingTop()) {
            ...
            mPreviewLayoutRule.init(mBackground.previewSize, mIntrinsicIconSize, Utilities.isRtl(getResources()));
    ...
    static class PreviewItemDrawingParams {
        ...
        public void update(float transX, float transY, float scale) {
            ...
            if (anim != null) {
                if (anim.finalTransX == transX || anim.finalTransY == transY || anim.finalScale == scale) {
                    return;
                }
                anim.cancel();
            }
    ...
    private PreviewItemDrawingParams computePreviewItemDrawingParams(int index, int curNumItems, PreviewItemDrawingParams params) {
    ...
    private void drawPreviewItem(Canvas canvas, PreviewItemDrawingParams params) {
        ...
        if (d != null) {
            ...
            if (d instanceof FastBitmapDrawable) {
                ...
                d.setColorFilter(Color.argb((int) (params.overlayAlpha * 255), 255, 255, 255), PorterDuff.Mode.SRC_ATOP);
                d.draw(canvas);
                d.clearColorFilter();
            }
    ...
    public static class PreviewBackground {
        ...
        private final RadialGradient mClipShader = new RadialGradient(0, 0, 1, new int[]{Color.BLACK, Color.BLACK, Color.TRANSPARENT}, new float[]{0, 0.999f, 1}, Shader.TileMode.CLAMP);
        ...
        public void setup(DisplayMetrics dm, DeviceProfile grid, View invalidateDelegate, int availableSpace, int topPadding) {
        ...
        public void drawBackground(Canvas canvas) {
            ...
            if (canvas.isHardwareAccelerated()) {
                saveCount = canvas.saveLayer(offsetX - mStrokeWidth, offsetY, offsetX + radius + shadowRadius, offsetY + shadowRadius + shadowRadius, null);//Canvas.CLIP_TO_LAYER_SAVE_FLAG | Canvas.HAS_ALPHA_LAYER_SAVE_FLAG

            } else {
                saveCount = canvas.save();// canvas.save(Canvas.CLIP_SAVE_FLAG);
        ...
        private void drawCircle(Canvas canvas, float deltaRadius) {
            float radius = getScaledRadius();
            canvas.drawCircle(radius + getOffsetX(), radius + getOffsetY(), radius - deltaRadius, mPaint);
        }
    ...
    protected void dispatchDraw(Canvas canvas) {
        ...
        if (canvas.isHardwareAccelerated()) {
            saveCount = canvas.saveLayer(0, 0, getWidth(), getHeight(), null);//Canvas.HAS_ALPHA_LAYER_SAVE_FLAG | Canvas.CLIP_TO_LAYER_SAVE_FLAG
        } else {
        ...
        if ((mBadgeInfo != null && mBadgeInfo.hasBadge()) || mBadgeScale > 0) {
            ...
            mBadgeRenderer.draw(canvas, badgePalette, mBadgeInfo, mTempBounds, badgeScale, mTempSpaceForBadgeOffset);
        }
    }
    ...
    class FolderPreviewItemAnim {
         ...
         */
        public FolderPreviewItemAnim(final PreviewItemDrawingParams params, int index0, int nItems0, int index1, int nItems1, int duration, final Runnable onCompleteRunnable) {
    ...
    private void animateFirstItem(final Drawable d, int duration, final boolean reverse, final Runnable onCompleteRunnable) {
        ...
        FolderPreviewItemAnim anim;
        if (!reverse) {
            anim = new FolderPreviewItemAnim(mDrawingParams.get(0), -1, -1, 0, 2, duration, onCompleteRunnable);
        } else {
            anim = new FolderPreviewItemAnim(mDrawingParams.get(0), 0, 2, -1, -1, duration, onCompleteRunnable);
        }
        anim.start();
    }
    ...
    private void updateItemDrawingParams(boolean animate) {
        ...
        for (int i = 0; i < mDrawingParams.size(); i++) {
            ...
            if (!animate || FeatureFlags.LAUNCHER3_LEGACY_FOLDER_ICON) {
                ...
                FolderPreviewItemAnim anim = new FolderPreviewItemAnim(p, i, prevNumItems, i, nItemsInPreview, DROP_IN_ANIMATION_DURATION, null);
