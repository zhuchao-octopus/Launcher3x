...
public class CircleSeekBar extends SeekBar {
	...
	private void initAttrs(AttributeSet attrs, int defStyle) {
		...
		mMaxProcess = a.getInt(R.styleable.CircleSeekBar_wheel_max_process,
				2000);
		mCurProcess = a.getInt(R.styleable.CircleSeekBar_wheel_cur_process,
				1500);
		...
		isHasReachedCornerRound = a.getBoolean(
				R.styleable.CircleSeekBar_wheel_reached_has_corner_round, true);
		mReachedWidth = a.getDimension(
		...
		mPointerRadius = a.getDimension(
				R.styleable.CircleSeekBar_wheel_pointer_radius,
				mReachedWidth / 2);
		isHasWheelShadow = a.getBoolean(
		...
		isHasPointerShadow = a.getBoolean(
		...
		isCanTouch = a.getBoolean(R.styleable.CircleSeekBar_wheel_can_touch,
				true);
		isScrollOneCircle = a.getBoolean(
	...
	private final static int OTHER_PADDING = 3;
	@Override
	protected void onDraw(Canvas canvas) {
		...
		if (isHasCache) {
			...
			canvas.drawArc(new RectF(left, top, right, bottom), -210, -120,
					false, mWheelPaint);
		}

		float endPos =  (float)(((mCurAngle*120/180)-60) - 120);//270~90 angle to 240 ~120

		Log.d("aaa", "mCurProcess" + mCurAngle + ":" + endPos);
		// 画选中区域
		canvas.drawArc(new RectF(left, top, right, bottom), -210,
				endPos, false, mReachedPaint);
...
//					&& (event.getAction() & (MotionEvent.ACTION_MOVE | MotionEvent.ACTION_UP)) > 0) {
//				mChangListener.onProgressChanged(this, mCurProcess, true);
//			}
//
//			if (mCurAngle > 270) {
//				mCurAngle = 270;
//			} else if (mCurAngle < 90) {
//				mCurAngle = 90;
//			} else {
//				invalidate();
//			}
...
//		}
	...
	private float calcXLocationInWheel(double angle, double cos) {
		if (angle < 180) {
			return (float) (getMeasuredWidth() / 2 -OTHER_PADDING+ Math.sqrt(1 - cos * cos)
					* mUnreachedRadius);
		} else {
			return (float) (getMeasuredWidth() / 2 +OTHER_PADDING- Math.sqrt(1 - cos * cos)
					* mUnreachedRadius);
	...
	private float calcYLocationInWheel(double cos) {
		return getMeasuredWidth() / 2 + OTHER_PADDING + mUnreachedRadius * (float) cos;
	}
	...
	// bundle.putBoolean(INSTANCE_WHEEL_SCROLL_ONLY_ONE_CIRCLE,
	// isScrollOneCircle);
	// return bundle;
	// }
	//
	...
	// isHasReachedCornerRound =
	// bundle.getBoolean(INSTANCE_REACHED_CORNER_ROUND);
	...
	// isScrollOneCircle =
	// bundle.getBoolean(INSTANCE_WHEEL_SCROLL_ONLY_ONE_CIRCLE);
	// initPaints();
	// } else {
	...
	public void setCurProcess(int curProcess) {
		...
		if (mChangListener != null) {
			mChangListener.onProgressChanged(this, curProcess, false);
		}
		refershPosition();
		invalidate();
	}

	public void setProcess(int curProcess) {
		if (curProcess >= 0 && curProcess <= 100) {
			setCurProcess(150 - curProcess);
		}
	}
	...
	public void setPointerShadowRadius(float pointerShadowRadius) {
		...
		if (mPointerShadowRadius == 0) {
			...
			mPointerPaint.setShadowLayer(pointerShadowRadius, mDefShadowOffset,
					mDefShadowOffset, Color.DKGRAY);
			setSoftwareLayer();
		}
		invalidate();
	...
	public void setProgress(int progress) {
		Log.d("ccd", "setProgress11:"+progress);
		progress = progress*10;
		progress = (1500 - progress);
		if (progress < 500) {
			progress = 500;
		} else if (progress > 1500) {
			progress = 1500;
		}
		Log.d("ccd", "setProgress22:"+progress);
		setCurProcess(progress);
	}

	public int getProgress() {
		return 1500 - mCurProcess;
	}
}
