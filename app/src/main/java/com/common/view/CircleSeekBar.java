...
public class CircleSeekBar extends SeekBar {
    ...
    private void initAttrs(AttributeSet attrs, int defStyle) {
        ...
        mMaxProcess = a.getInt(R.styleable.CircleSeekBar_wheel_max_process, 2000);
        mCurProcess = a.getInt(R.styleable.CircleSeekBar_wheel_cur_process, 1500);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (isHasCache) {
            ...
            canvas.drawArc(new RectF(left, top, right, bottom), -210, -120, false, mWheelPaint);
        }
        ...
        // 画选中区域
        canvas.drawArc(new RectF(left, top, right, bottom), -210, endPos, false, mReachedPaint);
    ...
    private float calcXLocationInWheel(double angle, double cos) {
        if (angle < 180) {
            return (float) (getMeasuredWidth() / 2 - OTHER_PADDING + Math.sqrt(1 - cos * cos) * mUnreachedRadius);
        } else {
            return (float) (getMeasuredWidth() / 2 + OTHER_PADDING - Math.sqrt(1 - cos * cos) * mUnreachedRadius);
        }
    }
